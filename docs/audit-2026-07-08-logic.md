...
- **[V] HIGH #1 — orphaned seller card hold (scheduled deals).** `holdRelease` gated on
  `sellerHeadedOut`, but scheduled deals place the deposit hold at `CONFIRM_MEETUP` (before
  head-out), so cancel/dispute/back-out never released it → the hold stuck on the seller's card.
...
- **[V] MED #3 — ASAP "stall trap".** If the counterparty ghosted (never headed out), the
  waiting party's only exit forfeited their *own* deposit. **Fix:** CANCEL from EN_ROUTE is a
...
- **[V] LOW #7 — stale seller copy.** Said the hold lands "when you head out"; scheduled
  deals place it at confirm. **Fix:** reworded to "a hold backs this meetup" (accurate for
  both modes; shows the actual deposit amount). (`AppContext.ts`, `dealLogic.ts`)
