-- Safety: an agreed meetup spot on the deal (a fair-by-time midpoint safe spot, or a
-- custom one). apply_transition persists the meetup fields from the deal JSON.
alter table public.deals
  add column if not exists meetup_name   text,
  add column if not exists meetup_lat    double precision,
  add column if not exists meetup_lng    double precision,
  add column if not exists meetup_custom boolean not null default false;
