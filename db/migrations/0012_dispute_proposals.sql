-- M?: self-service dispute resolution. Each party can PROPOSE a resolution; when both
-- propose the same outcome, the deal auto-resolves (no admin, no fault). Persist the
-- proposals on the disputes row; apply_transition handles the new dispute_proposal effect.
alter table public.disputes
  add column if not exists buyer_proposal  text check (buyer_proposal in ('release','refund','split')),
  add column if not exists seller_proposal text check (seller_proposal in ('release','refund','split'));
