-- M6: per-party "heading out" presence, so each phone can show the other side's
-- status. Adds two timestamps and teaches apply_transition to stamp them (same
-- pattern as *_arrived_at).
alter table public.deals
  add column if not exists buyer_headed_out_at  timestamptz,
  add column if not exists seller_headed_out_at timestamptz;
